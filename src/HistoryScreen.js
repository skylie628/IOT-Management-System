...
export default class HistoryScreen extends React.Component {
  ...
  componentDidMount() {
    ...
    firebase.database().ref('sensorHistory').orderByChild('date').limitToLast(7).on('child_added', (dataSnapshot) => {
      ...
      items.push(dataSnapshot.val().temp)
      ...
      var notice;
      var lasttemp = items[items.length - 1];
      if (lasttemp <= 20) notice = "thời tiết lạnh, nên bật điều hoà"
      else if (lasttemp <= 25) notice = "khí hậu mát mẻ"
      else if (lasttemp <= 30) notice = "nhiệt độ bình thường"
      else if (lasttemp <= 33) notice = "thời tiết oi bức, nên bật điều hoà và quạt"
      else notice = "cảnh báo nguy cơ cháy nổ, thiếu nước, nên bật điều hoà và quạt"
      this.setState({
        notice: notice,
      })
